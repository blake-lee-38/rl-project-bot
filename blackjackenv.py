...
class BlackjackEnv(gym.Env):
    """
    Modified Blackjack environment with a finite 6-deck shoe, cumulative rewards
    """
    ...
    # Shuffle Shoe, Reset Count
    def shuffle_shoe(self):
        self.shoe = self.single_deck * (4 * self.num_decks)
        self.count = 0
        np.random.shuffle(self.shoe)

    # Draw Card, Reshuffle shoe if necessary
    def draw_card(self):
        if len(self.shoe) < (self.num_decks * 52 * 0.2):  # Reshuffle if <20% cards left
            self.shuffle_shoe()
        return self.shoe.pop()

    # Draw Hand, update card count
    ...
    # Check if Ace is usable
    def usable_ace(self, hand):
        return 1 in hand and sum(hand) + 10 <= 21

    # Sum Hand, add 10 if Ace is usable
    ...
    # Get initial observation of new hand (player sum, dealer card, usable ace)
    def _get_obs(self):
        return (self.sum_hand(self.player), self.dealer[0], self.usable_ace(self.player))

    # Take action (hit or stick), return new observation, reward, done, info
    def step(self, action):
        assert self.action_space.contains(action)
        if action:  # hit

            # Draw card, update count
            ...
            # Dealer plays out hand, update count
            terminated = True
            ...
            # Determine hand winner, update reward
        ...
        # Return new observation, reward, done, info about hand/current state
    ...
    def reset(
        ...
        # Logic used to show dealer's top card in render
        ...
        # Return information about new hand and current state
    ...
    def render(self):
        ...
        if usable_ace:
            ...
            usable_ace_text_rect = self.screen.blit(
                usable_ace_text,
                (
            ...
            count_text = small_font.render("Count: " + str(self.count), True, white)
            self.screen.blit(
                count_text,
                (
                    screen_width // 2 - count_text.get_width() // 2,
                    usable_ace_text_rect.bottom + spacing // 2,
                ),
            )
        else:
            count_text = small_font.render("Count: " + str(self.count), True, white)
